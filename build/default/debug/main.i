...
# 16 "main.s" 2
 ...
 CALL antirrebote2
 BTFSC ((INTCON) and 07Fh), 2 ; Verificamos interrupción del timer0
 CALL inc_B
 GOTO loop ; Volvemos a comenzar con el loop

    ;----------------------------- SUBRUTINAS ----------------------------------
    COMPARACION:
 MOVF PORTB, 0 ; Guardamos caracter de CONT en W
 ANDLW CONT ; Hacemos un AND de ambos contadores
 MOVWF COMP
 BTFSS COMP, 0 ; Verificamos que el bit 0 de W sea 1
 return
 BTFSS COMP, 1 ; Verificamos que el bit 1 de W sea 1
 return
 BTFSS COMP, 2 ; Verificamos que el bit 2 de W sea 1
 return
 BTFSS COMP, 3 ; Verificamos que el bit 3 de W sea 1
 return
 CLRF PORTB
 CLRF COMP
 BSF PORTD, 0
 ...
 BSF OSCCON, 0 ; Configurar ((OSCCON) and 07Fh), 0 -> 1, Usamos reloj interno
 BCF OSCCON, 4
 BCF OSCCON, 5
 ...
 BCF TRISC, 5
 BCF TRISC, 6
 BCF TRISD, 0 ; Configurar como salida el bit 0 del PORTD
 BANKSEL PORTB ; Reiniciamos el PORTB
 CLRF PORTB
 CLRF PORTC
 BCF PORTD, 0 ; Reiniciamos el bit 0 del PORTD
 BSF PORTC, 0
 return
